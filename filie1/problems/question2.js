const products = [
  ...
  products.filter((elm)=>{
    if(elm.category==="Photography"){ 
         Photography.push(elm)
    }if(elm.category==="Fashion"){
         Fashion.push(elm)
    }if(elm.category==="Electronics"){
       Electronics.push(elm)
    }
    if(elm.category==="Furniture"){
         Furniture.push(elm)
    }
  });
  }  groupAndSortProducts (products)
  let obj={
